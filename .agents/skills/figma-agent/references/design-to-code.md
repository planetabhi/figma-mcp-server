...
1. Call `list_file_variables` for local variables, or `get_published_variables` for a published library. Variables need an Enterprise plan. If the call returns 403, fall back to styles.
2. If variables are not available, call `list_styles_in_file` to read shared colors, text styles, and effects.
