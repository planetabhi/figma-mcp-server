...
const executeFunction = async ({ file_key, group_by, cursor, start_date, end_date }: any) => {
  return figmaRequest(`/v1/analytics/libraries/${file_key}/variable/actions`, {
...
const apiTool: ApiTool = {
  ...
  definition: {
    type: 'function',
    function: {
      name: 'get_library_variable_actions',
      description: 'Get library analytics variable action data (detachments, insertions) broken down by the requested dimension.',
      parameters: {
        type: 'object',
        properties: {
          file_key: { type: 'string', description: 'File key of the library to fetch analytics for.' },
          group_by: { type: 'string', description: 'Dimension to group by: "variable" or "team".' },
